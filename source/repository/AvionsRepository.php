...
class AvionsRepository {
    ...
    public function createAvion(AvionModel $avion) {
        $query = "INSERT INTO avions (nom, type, capacite, compagnie) VALUES (:nom, :type, :capacite, :compagnie)";
    ...
    public function getAvion($nom) {
        ...
        if ($row) {
            return new AvionModel($row['id'], $row['nom'], $row['type'], $row['capacite'], $row['compagnie']);
        }
        return null;
    }
    ...
    public function updateAvion(AvionModel $avion) {
        $query = "UPDATE avions SET nom = :nom, type = :type, capacite = :capacite, compagnie = :compagnie WHERE id = :id";
        $stmt = $this->pdo->prepare($query);
        $stmt->execute([
            ':nom' => $avion->getNom(),
            ':type' => $avion->getType(),
