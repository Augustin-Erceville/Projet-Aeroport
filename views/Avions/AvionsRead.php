...
$avions = [];
try {
     $stmt = $bdd->query("SELECT * FROM V_avions");
...
<div class="mx-4">
     <div class="row">
          ...
          <a href="AvionsCreate.php" class="btn btn-outline-success text-uppercase">Ajouter un vol</a>
     </div>
</div>
          ...
          <?php if (count($avions) > 0) : ?>
               <?php foreach ($avions as $avion) : ?>
                    <tr>
                         ...
                         <td><?= htmlspecialchars($avion['Immatriculation']) ?></td>
                         <td><?= htmlspecialchars($avion['Modèle']) ?></td>
                         <td><?= htmlspecialchars($avion['Capacité']) ?></td>
                         <td><?= htmlspecialchars($avion['Compagnie']) ?></td>
                         <td>
                              <a href="AvionsUpdate.php?id=<?= $avion['ID'] ?>" class="btn btn-warning btn-sm">✒️</a>
                              <a href="AvionsDelete.php?id=<?= $avion['ID'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet avion ?');">🗑️</a>
                         </td>
                    </tr>
               <?php endforeach; ?>
          <?php else : ?>
               <tr>
                    <td colspan="6" class="text-center">Aucun avion trouvé.</td>
               </tr>
          <?php endif; ?>
          </tbody>
     </table>
